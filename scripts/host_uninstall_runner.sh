...
exec >>'$LOG_FILE' 2>&1
echo "[\$(date -u +%Y-%m-%dT%H:%M:%SZ)] [oneshot] starting bash $UNINSTALL_SCRIPT $UNINSTALL_FLAGS"
if bash '$UNINSTALL_SCRIPT' $UNINSTALL_FLAGS; then
    echo "[\$(date -u +%Y-%m-%dT%H:%M:%SZ)] [oneshot] uninstall.sh succeeded; tearing down sibling LaunchAgents"
    for plist in \\
        '$PLIST_PATH' \\
        '$HOME/Library/LaunchAgents/com.sygen.host-update-runner.plist' \\
        '$HOME/Library/LaunchAgents/com.sygen.host-updates-check.plist'
    do
    ...
    rm -f '$TEMP_PLIST' '$ONESHOT_WRAPPER' 2>/dev/null
else
